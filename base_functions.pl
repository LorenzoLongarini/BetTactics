...
winner_home(X,Result):-findall(X,matchSA(X,_,"HOME_TEAM",_,_),Result).
num_winner_home(X,Count):-winner_home(X,Result),length(Result,Count).

winner_away(X,Result):-findall(X,matchSA(_,X,"AWAY_TEAM",_,_),Result).
num_winner_away(X,Count):-winner_away(X,Result),length(Result,Count).
